...
from Layers.my_graph_layer import GCN, GraphUnet, Initializer, norm_g
...
class GNet(nn.Module):
    ...
    def forward(self, gs, hs, es):
        # gs = torch.tensor(gs.adjacency_matrix())
        src, dst = gs.edges()
        block_adj = torch.zeros(gs.num_src_nodes(), gs.num_dst_nodes())
        block_adj[src, dst] = gs.edata['feat'].squeeze(-1)
        A = block_adj
        hs = self.embed(A, hs)
