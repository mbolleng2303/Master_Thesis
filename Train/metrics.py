...
def accuracy_SBM(scores, targets):
    ...
    return acc
def dice(scores, targets):
    y_true = targets.cpu().detach().numpy()
    y_pred = scores.detach().argmax(dim=1).cpu()
    j= jaccard_score(y_true, y_pred,average='weighted')
    dice = 2 * j / (j + 1)
    '''y_pred = []
    ...
    j = jaccard_score(y_true, y_pred,average='samples')
    dice = 2*j/(j+1)'''
    return dice
